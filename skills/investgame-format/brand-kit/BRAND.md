...
The real logos live in `assets/logos/`: a full-colour mark (`ig-logo.png`, `ig-logo-336x88.png`) for light/warm backgrounds, and a white logotype (`ig-logotype-white.png` / `.webp`) for dark backgrounds. Both themes carry the logo — **Dark Navy** uses the white logotype on its cover and footer; **Warm White** uses the colour logo on its cover and a small colour mark in the slide corner. Give it clear space, and never recolour, stretch, rotate, or add effects. Clear-space, minimum sizes, and on-light/on-dark rules are in **`references/logos.md`**.
